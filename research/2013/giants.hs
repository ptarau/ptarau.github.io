...
-- is required before importing this
...
instance N T where
  ...
  -- more efficient successor function
  s T = V T []
  s (V T []) =  W T []
  s (V T (x:xs)) =  W (s x) xs  
  s (V t xs) = W T (s' t : xs)
  s (W t []) = V (s t) []
  s (W t (x:xs)) = V t (t' : xs') where
    W t' xs' = s (V x xs)

  -- more efficient predecessor function
  s' (V T []) = T
  s' (V t []) = W (s' t) []
  s' (V t (x:xs)) =  W t (t' : xs') where
     V t' xs' = s' (W x xs)
  s' (W T []) = V T []
  s' (W T (x:xs)) = V (s x) xs where  
  s' (W t xs) = V T (s' t : xs)
...
class Collections n => BitwiseOperations n where 
  ...
  l_ite :: n->n->n->n
  l_ite x y z = 
   from_set (ite (to_set x) (to_set y) (to_set z)) where
...
class SpecialComputations n => Benchmarks n where
  ...
  primes = 
   s (s e) : filter is_prime (odds_from three) where
    three = s (s (s e)) 
...
bm1t = benchmark "ack 3 7 on t" 
  (ack (t (toInteger 3)) (t (toInteger 7)))
bm1b = benchmark "ack 3 7 on b" 
  (ack (b (toInteger 3)) (b (toInteger 7)))
bm1n = benchmark "ack 3 7 on n" 
  (ack (n (toInteger 3)) (n (toInteger 7)))

bm2t = benchmark "exp2 t" 
  (exp2 (exp2 (t (toInteger 14))))
bm2b = benchmark "exp2 b" 
  (exp2 (exp2 (b (toInteger 14))))
bm2n = benchmark "exp2 n" 
  (exp2 (exp2 (n (toInteger 14))))

bm3 tvar = benchmark "sparse_set on a type" 
 (n (bitsize (from_set ps))) where 
   ps = map tvar [101,2002..100000]

bm3' tvar m = benchmark "to/from list on a type" 
 (n (bitsize (from_list (to_list (from_list ps))))) where 
   ps = map tvar [101,2002..3000+m]

bm3'' tvar m = benchmark "to/from list on a type" 
 (n (bitsize (from_list (to_list (from_list ps))))) where 
   ps = map (dual.tvar) [101,2002..3000+m]

bm7t = benchmark "primes on t" (last (take 100 ps)) where 
   ps = primes :: [T]
...
bm8n = benchmark "mersenne on n" 
  (last (take 7 ps)) where 
    ps = mersenne_primes :: [Integer]

kth _ k x | e_ k  = x
kth f k x  = f (kth f (s' k) x)

-- assertions about iterations of o,i and power of 2 
a1 k = pow (i e) k == s (kth o k e) 
a2 k = pow (i e) k == s (s (kth i (s'  k) e))

-- assertions about power operations and multiplication

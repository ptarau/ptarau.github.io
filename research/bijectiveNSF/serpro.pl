...
toTypeValue(P,TV):-integer(P),P>=0,!,Code=P,TV=leaf(2,Code).
toTypeValue(P,TV):-integer(P),P<0,!,Code is -P,TV=leaf(3,Code).  

fromTypeValue(leaf(0,N),P):-!,P='$VAR'(N).
fromTypeValue(leaf(1,Code),P):-!,nat2atom(Code,P).
fromTypeValue(leaf(2,Code),P):-!,P is Code.
fromTypeValue(leaf(3,Code),P):-!,P is -Code.
...
putBDigit(B,D,M,R):-D>=0,D < B, R is 1+D+B*M.

getBDigit(B,N,Digit,NewN):-N>0,Q is N // B,D is N mod B,
...
term2bitpars(T,[0|Ps],NewPs)-->{T=fun(F,Xs)},[fun(F)],
...
combinatoriallDigits(K,N,Ds,NewDs):-K > 0,K1 is K-1,
  upperBinomial(K,N,M),
  M1 is M-1,binomial(M1,K,BDigit),N1 is N-BDigit,
...
encodeSyms([leaf(T,C)|Xs],[N|Ns]):-!,
  totalTypes(B),putBDigit(B,T,C,N1),N is N1-1,
  encodeSyms(Xs,Ns).

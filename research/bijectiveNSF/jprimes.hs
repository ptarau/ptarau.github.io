...
data (Integral a) => P a = P a deriving (Show, Read, Eq, Ord)
...
{-
boshow t m = plotl (bigomega_sum t m)
oshow t m = plotl (omega_sum t m)

show_omega m = plotls [omega_sum X2 m,
  omega_sum X3 m,omega_sum P m] -- red,yellow,blue

show_bigomega m = plotls [bigomega_sum X2 m,
  bigomega_sum X3 m,bigomega_sum P m] -- red,yellow,blue
...
lens t m = map (length . unmix . t) [1..2^m]
lens_sum t m =  scanl (+) 0 (lens t m)  

show_lens m = plotls [lens_sum X2 m,
  lens_sum X3 m,lens_sum P m] -- red,yellow,blue
-}
  
{-
xrad t n = unlift tr where
   ms=as_xmset (t n)
   ns=nub ms
   tr=mix (mset2list (map t ns))

xrads t m = map (xrad t) (lifts [1..2^m])
-}

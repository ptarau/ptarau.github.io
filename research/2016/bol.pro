...
multi_gen_nf(Res):-
  Res=[X,T,Size,Steps],
  prolog_flag(cpu_count,MaxThreads), 
  G=ranTypableNF(X,T,Size,Steps),
  length(Goals,MaxThreads),
  maplist(=(G),Goals),
  first_solution(Res,Goals,[on_fail(continue)]).

% for this, you might want ot lift size limits up by 10-20
mgen:-Res=[X,T,Size,Steps],
  time(multi_gen(Res)),
  ppp(term=X),ppp(type=T),ppp(size(Size)+steps(Steps)).
  
mgen_nf:-Res=[X,T,Size,Steps],
  time(multi_gen_nf(Res)),
  ppp(term=X),ppp(type=T),ppp(size(Size)+steps(Steps)).  
...
rgen_nf:-time(ranTypableNF(X,T,Size,Steps)),
   ppp(term=X),ppp(type=T),ppp(size(Size)+steps(Steps)),
   fail.   

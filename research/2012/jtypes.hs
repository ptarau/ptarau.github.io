...
class PureTypes n=>PeanoArith n where 
  ...
  s z | isEmpty (from z) = arrow (s (from st)) (to st) where
    st = s (to z)
...
class PeanoArith n=>GoedelArith n where
  ...
  recPow x y = itr f y (s empty) where f y = recMul x y
...
instance GoedelArith T
instance GoedelArith N

class PeanoArith n=>TreeArith n where
  one :: n
  ...
  sub :: n->n->n
  sub x y | isEmpty y = x
  sub y x | isOdd y && isOdd x = p (makeOdd
     (sub (trim y) (trim x)))  
  sub y x | isOdd y && isEven x = p (p (makeOdd
     (sub (trim y) (trim x))))
  sub y x | isEven y && isOdd x = makeOdd
     (sub (trim y) (trim x))  
  sub y x | isEven y && isEven x = p (makeOdd
     (sub (trim y) (trim x))) 
  ...
  log2 :: n->n
  log2 x | isEmpty (to x) = from x
  log2 x = add (from x) (s (log2 (to x)))
  

instance TreeArith T 
instance TreeArith N


class TreeArith n => Ordered n where
  cmp :: n->n->Ordering

...
uTreeReduce(OBDT,obdt(NV,R)):-nonvar(OBDT),OBDT=obdt(NV,X),uTreeReduce1(X,R).
...
reducedUnpairingTree(NV,TT, ReducedOBDT):-
  unPairingTree(NV,TT, OBDT),
  uTreeReduce(OBDT,ReducedOBDT).

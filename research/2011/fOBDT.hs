...
eval_with_mask m n (D x l r) = ite_ (vn n x) (eval_with_mask m n l) 
...
nat2obdt n = unfold_obdt (k,n_m) where (k,n_m) = to_bsum n

obdt2nat obdt@(OBDT nv _) =  (bsum nv)+tt where(_,tt) = fold_obdt obdt

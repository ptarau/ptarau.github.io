...
% eval_bitstring_int(1,NbOfBits,X):-!,all_ones_mask(NbOfBits,X).
...
vars_to_bitstring_ints(Us):-vars_to_bitstring_ints(Us,_NbOfBits).

% bind N free vars to their N-bit bitstring_int representation
vars_to_bitstring_ints(Us,NbOfBits):-
  length(Us,NbOfBits),
  vars_to_bitstring_ints(Us,NbOfBits,0,NbOfBits).

vars_to_bitstring_ints([],_,N,N).
vars_to_bitstring_ints([X|Xs],NbOfBits,N1,N2):-
  N is N1+1,
  var_to_bitstring_int(NbOfBits,N1,X),
  vars_to_bitstring_ints(Xs,NbOfBits,N,N2).
...
biteq(NbOfBits,X1,X2,X3):-all_ones_mask(NbOfBits,M),X3 is '#'(M,'#'(X1,X2)).
...
% replaces 1 with 1...1 and expresses ~ as binary op (A=>B)
replace_one_with(_,V,R):-var(V),!,R=V.
replace_one_with(One,1,R):-!,R=One.
replace_one_with(_,0,R):-!,R=0.
replace_one_with(One,~(X),(Y=>0)):- !,
  replace_one_with(One,X,Y).
replace_one_with(One,T,R):-
  T=..[Op|Xs],
  map(replace_one_with(One),Xs,Ys),
  R=..[Op|Ys].
